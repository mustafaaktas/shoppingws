	...
	public void logwrite(){
		...
		for(StackTraceElement er:errs){ 			
			...
			if (strClassName == null || strClassName.length() == 0
					|| strClassName.indexOf("BySpringCGLIB") > -1
					|| strClassName.indexOf("TokenController") > -1
					||(strClassName.indexOf("com.vor.dcsws") == -1
					&& strClassName.indexOf("PageExpiryFilter") == -1))
				continue;
			
			
			if(1==2 && "true".equals(useElasticLogger)){
				error = new ErrorData();
				...
				errorList.add(error);
				Helper.log_error("DCSWS : " + "|" + serverName + "|" + strDate+"|" + time + "|" + strFileName + "|" + strClassName + "|" +strMethodName+ "|" + strLineNumber + "|" + userName+ "|" + strErrorMessage + "|" + exception + "|" + extraInfo);
